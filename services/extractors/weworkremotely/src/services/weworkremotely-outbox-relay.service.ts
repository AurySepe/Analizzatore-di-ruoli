...
@Injectable()
export class WeWorkRemotelyOutboxRelayService {
  private readonly logger = new Logger(WeWorkRemotelyOutboxRelayService.name);
  ...
  async processPendingEvents(): Promise<number> {
    const pendingJobs = await this.prisma.weWorkRemotelyScrapedJob.findMany({
    ...
    this.logger.log(`📬 [WeWorkRemotely Relay] Invio di ${pendingJobs.length} offerte a BullMQ...`);
    ...
    for (const job of pendingJobs) {
      try {
        ...
        await this.prisma.weWorkRemotelyScrapedJob.update({
          where: { id: job.id },
          data: {
        ...
        this.logger.error(`❌ Errore pubblicazione offerta WeWorkRemotely ${job.externalId} su BullMQ: ${err.message}`);
        await this.prisma.weWorkRemotelyScrapedJob.update({
          where: { id: job.id },
          data: {
    ...
    this.logger.log(`✅ [WeWorkRemotely Relay] ${publishedCount} eventi pubblicati con successo.`);
