...
@Injectable()
export class HimalayasOutboxRelayService {
  private readonly logger = new Logger(HimalayasOutboxRelayService.name);
  ...
  async processPendingEvents(): Promise<number> {
    const pendingJobs = await this.prisma.himalayasScrapedJob.findMany({
    ...
    this.logger.log(`📬 [Himalayas Relay] Invio di ${pendingJobs.length} offerte a BullMQ...`);
    ...
    for (const job of pendingJobs) {
      try {
        ...
        this.logger.error(`❌ Errore pubblicazione offerta Himalayas ${job.externalId} su BullMQ: ${err.message}`);
        await this.prisma.himalayasScrapedJob.update({
          where: { id: job.id },
          data: {
    ...
    this.logger.log(`✅ [Himalayas Relay] ${publishedCount} eventi pubblicati con successo.`);
