...
export class ArbeitnowOutboxRelayService {
  ...
  async processPendingOutbox(batchSize = 50): Promise<number> {
    ...
    this.logger.log(`📤 [OutboxRelay] Trovati ${pendingJobs.length} annunci PENDING da inviare a BullMQ...`);
    ...
    for (const job of pendingJobs) {
      try {
        ...
        this.logger.error(`❌ Errore outbox relay per job ${job.id}:`, err.message);
        await this.prisma.arbeitnowScrapedJob.update({
          where: { id: job.id },
          data: {
