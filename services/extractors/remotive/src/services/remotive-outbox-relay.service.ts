...
@Injectable()
export class RemotiveOutboxRelayService {
  private readonly logger = new Logger(RemotiveOutboxRelayService.name);
  ...
  async processPendingEvents(): Promise<number> {
    const pendingJobs = await this.prisma.remotiveScrapedJob.findMany({
    ...
    this.logger.log(`📬 [Remotive Relay] Invio di ${pendingJobs.length} offerte a BullMQ...`);
    ...
    for (const job of pendingJobs) {
      try {
        ...
        await addSafeQueueJob(this.ingestionQueue, JOB_OFFER_SCRAPED_EVENT, payload, {
          jobId: `remotive-${job.externalId}`,
          attempts: 3,
          backoff: { type: 'exponential', delay: 1000 },
          removeOnComplete: true,
        });

        await this.prisma.remotiveScrapedJob.update({
          where: { id: job.id },
          data: {
        ...
        this.logger.error(`❌ Errore pubblicazione offerta ${job.externalId} su BullMQ: ${err.message}`);
        await this.prisma.remotiveScrapedJob.update({
          where: { id: job.id },
          data: {
    ...
    this.logger.log(`✅ [Remotive Relay] ${publishedCount} eventi pubblicati con successo.`);
