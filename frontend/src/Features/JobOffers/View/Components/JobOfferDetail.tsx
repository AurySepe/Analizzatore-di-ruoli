...
}> = ({ data, isFetching, onClearSelection }) => {
  if (data === null) {
    return (
    ...
    <aside className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="border-b border-slate-200 p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-sky-600">Dettaglio annuncio</p>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950">{data.title}</h2>
            <p className="mt-2 text-base text-slate-600">{data.company.name}</p>
          </div>
          <button
            type="button"
      ...
      <div className="max-h-[820px] space-y-8 overflow-y-auto p-6">
        ...
        <section>
          <h3 className="text-lg font-semibold text-slate-950">Descrizione completa</h3>
...
export const JobOfferDetail: React.FC<{
  ...
  switch (state.status) {
    ...
    case 'success':
      return <JobOfferDetailView data={state.data} isFetching={state.isFetching} onClearSelection={onClearSelection} />;
  }
};
