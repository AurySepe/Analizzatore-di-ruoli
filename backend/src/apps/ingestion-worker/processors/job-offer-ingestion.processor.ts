...
export class JobOfferIngestionProcessor extends WorkerHost {
  ...
  async process(job: Job<ScrapedJobOfferEvent, any, string>): Promise<any> {
    ...
    this.logger.log(`📥 [IngestionWorker] Ricevuto annuncio da coda: "${payload.title}" (${payload.companyName}) - Fonte: ${payload.source}`);
    ...
    try {
      ...
      if (!existingOffer) {
        ...
        this.logger.log(`✨ [IngestionWorker] Creato nuovo annuncio nel DB principale [ID: ${newOffer.id}] "${newOffer.title}"`);
      } else {
        ...
        this.logger.log(`🔄 [IngestionWorker] Aggiornato annuncio esistente [ID: ${existingOffer.id}] "${existingOffer.title}"`);
      }

      return { status: 'SUCCESS', url: payload.url };
    } catch (err: any) {
      this.logger.error(`❌ Errore ingestion annuncio "${payload.title}" (${payload.url}):`, err.message);
      throw err; // Permette a BullMQ di eseguire i retry configurati
    }
  }
}
