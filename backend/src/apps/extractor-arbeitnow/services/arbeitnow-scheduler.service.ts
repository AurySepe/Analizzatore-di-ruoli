...
export class ArbeitnowSchedulerService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log('🚀 Avvio del microservizio Arbeitnow Extractor...');
    // Esegue una prima estrazione all'avvio
  ...
  async handleOutboxRelayCron() {
    // Esegue periodicamente il relay dei record PENDING
  ...
  async runExtractionCycle() {
    ...
    if (syncState.status === 'RUNNING') {
      this.logger.warn('⚠️ Ciclo di estrazione precedente ancora in RUNNING. Salto questo turno.');
      return;
    }
